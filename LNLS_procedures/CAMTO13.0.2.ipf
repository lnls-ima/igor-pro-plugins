...
Function Add_Field_Profile()
	...
	if (DimSize(NormalMultipoles, 0))	
		...
		Notebook Report, text="\t",scaling={110,110},picture={$("CompareFieldInLine_" + FieldComponent),-9, 1, 8},text="\r"
		Killwindow/Z $("CompareFieldInLine_" + FieldComponent)

		Compare_Field_Profile_(FieldComponent=FieldComponent)
		Notebook Report, text="\t",scaling={110,110},picture={$("CompareFieldProfile_" + FieldComponent),-9, 1, 8},text="\r"
...
Function Add_Multipole_Profile(K, FieldComponent, Dynamic)
	...
	if (Dynamic)
		...
		Notebook Report, text="\t",scaling={90,90}, picture={$("CompareDynMult" + FieldComponent),-9, 1, 8},text="\r"
		Killwindow/Z $("CompareDynMult" + FieldComponent)
	else
		Compare_Multipole_Profile_(0, K=K, FieldComponent=FieldComponent)
		Notebook Report, text="\t",scaling={90,90}, picture={$("CompareMult" + FieldComponent),-9, 1, 8},text="\r"
...
Function Add_Residual_Field_Profile(Dynamic)
	...
	Notebook Report, text="\t", picture={$("NormalResidualField"),-9, 1, 8}, text="\r\r"
	Killwindow/Z $("NormalResidualField")

	Notebook Report, text="\t", picture={$("SkewResidualField"),-9, 1, 8}, text="\r\r"
...
Function LoadFieldmaps()
	...
	string message, outputPaths, filename, defaultFolderName
	...
	for(i=0; i<ItemsInList(outputPaths, "\r"); i+=1)
		...
		defaultFolderName = GetDefaultFieldmapFolderName_Mod(filename)
		CreateFieldmapFolder_Mod(defaultFolderName, replace=1)
		Carrega_Resultados_Mod(filename)
	endfor	
		
End


Static Function CreateFieldmapFolder_Mod(folderName, [replace])
	string folderName
	variable replace
	...
	SVAR fieldmapFolder = root:varsCAMTO:FieldmapDir
	NVAR fieldmapCount = root:varsCAMTO:FieldMapCount
	WAVE/T fieldmapFolders = root:wavesCAMTO:FieldmapDirs
...
Function/S GetDefaultFieldmapFolderName_Mod(filename)
	string filename
...
Function Carrega_Resultados_Mod(filename)
	String filename
...
Function PlotForAllFieldmaps(ywave, xwave)
	string ywave, xwave
	...
	for (i=0; i<FieldMapCount; i=i+1)
		...
		Wave tempx = root:$(df):$(xwave)
		Wave tempy = root:$(df):$(ywave)
		if (WaveExists(tempx) && WaveExists(tempy))
			AppendToGraph/W=$(plotname) tempy/TN=$(df) vs tempx
		else
			Print("Wave not found for: " + df)
			break
		endif
